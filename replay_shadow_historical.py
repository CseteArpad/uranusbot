...
def _ts(c) -> int:     return int(c[0])       # milliseconds
def _ts_sec(c) -> int: return int(c[0]) // 1000  # seconds (for timestamps)
...
# ---------------------------------------------------------------------------
# Rule engine context builder (cfg params, not env vars)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Profit guard (cfg params, not env vars)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Unrealised PNL (cfg params, not env vars)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Single tick
# candle_ts_sec: the candle's own timestamp in seconds (not wall clock).
# Used to stamp entry_ts / exit_ts so ledger reflects historical time.
# ---------------------------------------------------------------------------

def run_tick(shadow: dict, market: dict, cfg: dict, candle_ts_sec: int) -> Optional[dict]:
    """
    Run one replay tick. Mutates shadow in-place.
    Returns a completed ledger entry dict on SELL, else None.

    candle_ts_sec must be the candle's timestamp // 1000 so that all
    ledger timestamps reflect historical time, not script wall-clock.
    """
    last = _sf(market.get("last"))
    if last is None:
        return None
    ...
    # 3 — Rule engine context (cfg params, no env var reads)
    try:
    ...
    if act == "BUY" and not in_pos:
        ...
        if equity is not None and equity > 0:
            ...
            # FIX: _shadow_record_buy stamps entry_ts = int(time.time()) (wall clock).
            # Overwrite immediately with the candle's historical timestamp.
            shadow["entry_ts"] = candle_ts_sec
        ...
        if guard_ok:
            ...
            # Save entry_ts before _shadow_record_sell clears it.
            # This was already corrected to a candle ts on the BUY tick.
            entry_ts_for_ledger = shadow.get("entry_ts")

            ledger_entry = sp._shadow_record_sell(shadow, last, decision, fee_pct, slippage_pct)

            if ledger_entry:
                # FIX: _shadow_record_sell stamps exit_ts = int(time.time()) (wall clock)
                # and computes duration_sec = exit_ts - entry_ts, both wrong.
                # Patch all three fields with historical timestamps.
                ledger_entry["exit_ts"]      = candle_ts_sec
                ledger_entry["entry_ts"]     = entry_ts_for_ledger
                ledger_entry["duration_sec"] = (
                    (candle_ts_sec - entry_ts_for_ledger)
                    if entry_ts_for_ledger is not None
                    else 0
                )
...
def _final_equity(shadow: dict, cfg: dict) -> Tuple[float, bool]:
    """
    Return (equity_usdc, ends_in_position).

    When the replay ends mid-position, shadow["equity_usdc"] is 0.0 because
    _shadow_record_buy zeroed it out. Use mark-to-market instead:
        net_proceeds = entry_qty_base * last_close * (1 - fee_pct)
    which equals entry_cost + unrealised["net_usdc"] computed on the last tick.
    """
    in_pos = bool(shadow.get("in_position", False))
    if not in_pos:
        return (_sf(shadow.get("equity_usdc")) or 0.0), False

    entry_cost = _sf(shadow.get("entry_cost_usdc")) or 0.0
    net_u      = _sf((shadow.get("unrealised") or {}).get("net_usdc")) or 0.0
    mtm        = entry_cost + net_u
    return mtm, True
...
def run_replay(cfg: dict, verbose: bool = False) -> dict:
    ...
    # Slice to last N days + MA warmup
    ...
    print(
        f"[replay] Window: {first_dt.date()} to {last_dt.date()} "
    ...
    for i in range(replay_offset, len(working)):
        candle      = working[i]
        candle_ts   = _ts_sec(candle)          # historical seconds — passed to run_tick
        market      = _build_market(i, working, all_closes, cfg)
        ledger_entry = run_tick(shadow, market, cfg, candle_ts)
        tick_count  += 1

        # Track equity curve per tick
        ...
        if ledger_entry:
            ...
            if verbose:
                ...
                e_dt    = datetime.fromtimestamp(
                    ledger_entry["entry_ts"], tz=timezone.utc
                ).strftime("%Y-%m-%dT%H:%MZ") if ledger_entry.get("entry_ts") else "?"
                x_dt    = datetime.fromtimestamp(
                    ledger_entry["exit_ts"], tz=timezone.utc
                ).strftime("%Y-%m-%dT%H:%MZ") if ledger_entry.get("exit_ts") else "?"
                dur     = ledger_entry.get("duration_sec", 0)
                print(
                    f"  [{side}] #{ledger_entry.get('trade_id')} "
                    f"{e_dt} to {x_dt} ({dur}s) "
    ...
    # --- Final equity: account for open position at end of replay ---
    final_eq, ends_in_position = _final_equity(shadow, cfg)

    if ends_in_position:
        open_entry_price = _sf(shadow.get("entry_price"))
        open_entry_cost  = _sf(shadow.get("entry_cost_usdc"))
        open_entry_rule  = shadow.get("entry_rule")
        open_entry_ts    = shadow.get("entry_ts")
        last_price       = _sf(market.get("last"))  # type: ignore[possibly-undefined]
        print(
            f"[replay] Ends IN POSITION — entry={open_entry_price} "
            f"cost={open_entry_cost} rule={open_entry_rule} "
            f"mark-to-market equity={final_eq:.6f} USDC",
            flush=True,
        )
    ...
    open_position_info: Optional[dict] = None
    if ends_in_position:
        open_position_info = {
            "entry_price":    _sf(shadow.get("entry_price")),
            "entry_cost_usdc": _sf(shadow.get("entry_cost_usdc")),
            "entry_rule":     shadow.get("entry_rule"),
            "entry_ts":       shadow.get("entry_ts"),
            "entry_ts_utc":   (
                datetime.fromtimestamp(shadow["entry_ts"], tz=timezone.utc)
                .isoformat().replace("+00:00", "Z")
                if shadow.get("entry_ts") else None
            ),
            "mark_to_market_usdc": final_eq,
            "unrealised_net_usdc": _sf((shadow.get("unrealised") or {}).get("net_usdc")),
            "unrealised_pct":      _sf((shadow.get("unrealised") or {}).get("pct")),
        }
    ...
    return {
        ...
        "replay_window": {
            ...
            "ends_in_position":  ends_in_position,
        },
        "summary": {
...
def _fmt_trade(t: dict) -> List[str]:
    ...
    dur_str  = f"{dur // 3600}h{(dur % 3600) // 60}m" if dur and dur >= 60 else f"{dur}s"
    lines    = [f"  #{t.get('trade_id', '?')} [{side}] duration={dur_str}"]
    if entry_ts:
...
def format_txt_report(result: dict) -> str:
    ...
    if op:
        ...
        lines += [
            ...
            f"  Cost           : {op.get('entry_cost_usdc', 0):.4f} USDC",
            f"  Unrealised net : {op.get('unrealised_net_usdc', 0):+.6f} USDC  ({unr_pct:+.4f}%)",
            f"  Mark-to-market : {op.get('mark_to_market_usdc', 0):.6f} USDC",
        ]
...
def main(argv: Optional[List[str]] = None) -> None:
    # Reconfigure stdout/stderr to UTF-8 on Windows so box-drawing chars in
    # the report don't crash the console (cp1250 cannot encode them).
